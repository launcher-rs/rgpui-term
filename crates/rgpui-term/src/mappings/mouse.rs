...
use rgpui::{Modifiers, MouseButton, Pixels, Point, ScrollWheelEvent, px};
...
impl AlacMouseButton {
    fn from_move_button(e: Option<MouseButton>) -> Self {
        match e {
            Some(rgpui::MouseButton::Left) => AlacMouseButton::LeftMove,
            Some(rgpui::MouseButton::Middle) => AlacMouseButton::MiddleMove,
            Some(rgpui::MouseButton::Right) => AlacMouseButton::RightMove,
            Some(rgpui::MouseButton::Navigate(_)) => AlacMouseButton::Other,
    ...
    fn from_button(e: MouseButton) -> Self {
        match e {
            rgpui::MouseButton::Left => AlacMouseButton::LeftButton,
            rgpui::MouseButton::Right => AlacMouseButton::MiddleButton,
            rgpui::MouseButton::Middle => AlacMouseButton::RightButton,
            rgpui::MouseButton::Navigate(_) => AlacMouseButton::Other,
        }
    }

    fn from_scroll(e: &ScrollWheelEvent) -> Self {
        let is_positive = match e.delta {
            rgpui::ScrollDelta::Pixels(pixels) => pixels.y > px(0.),
            rgpui::ScrollDelta::Lines(lines) => lines.y > 0.,
        };
